...
- 2026-04-10 — Playback handoff + Originals foundation: added `url_launcher` and `just_audio`, implemented platform handoff service for Spotify/Apple Music/YouTube Music fallback launching from Player and Media Source track actions, and introduced a global Originals audio service with in-app Home playback controls streaming from a Cloudflare R2 base URL define.
- 2026-04-10 — Finalized Auth0 callbacks: added Auth0 intent-filter to Android AndroidManifest.xml to handle deep-link callbacks with scheme `com.ostrack.ostrackapp` and host `callback`; verified iOS Info.plist CFBundleURLTypes configuration; confirmed all pubspec.yaml dependencies (auth0_flutter, flutter_riverpod, go_router, supabase_flutter, typesense, url_launcher, just_audio) are in place.
- 2026-04-10 — Implemented optimistic mutations & background queues: created `MutationQueue` system with exponential backoff retry logic (2s → 4s → 8s → 30s cap), added Isar schema models for shelf operations (IsarShelf, IsarShelfTrack, IsarTrackRating, IsarPendingMutation), implemented `SupabaseShelfRepository` with optimistic UI updates and Cache-Then-Network pattern, wired Riverpod providers for shelves/shelf-tracks/pending-mutations, and added `ShelfController` for high-level mutation API; all mutations now persist in Isar and auto-retry on network recovery; Isar boilerplate generated successfully with build_runner.
- 2026-04-10 — Added unified offline banner: created `OfflineBoundary` widget that wraps main navigation shell, streams connectivity status from `connectivity_plus` via `Connectivity().onConnectivityChanged`, and displays coral-colored animated banner ("Offline mode. Browsing local cache.") when device loses network; banner slides in/out smoothly over 300ms with `AnimatedPositioned`, positioned above content with `SafeArea` respect to avoid notch/status-bar collision, and wrapped entire shell navigation in app_router to provide app-wide offline UX awareness.
- 2026-04-10 — Implemented the cinematic player screen: created high-performance `CinematicPlayerScreen` with glassmorphism aesthetic (BackdropFilter blur, LinearGradient overlay), detached StreamBuilder-driven playback UI from Riverpod state to maintain 60fps during audio playback, wired `ProgressBar` and playback controls directly to `just_audio` engine via positionStream (no Riverpod rebuilds for progress updates), added Hero animation for album artwork, scene timeline context tag, platform badge, and full-screen modal integration; added `audio_video_progress_bar`, `palette_generator`, and `cached_network_image` packages; created `CINEMATIC_PLAYER_INTEGRATION.md` with wiring guide for mini-player tap, explore track cards, and home playback cards.
- 2026-04-10 — Integrated palette_generator for dynamic album-aware UI: created `TrackPalette` model with dominant/vibrant/muted color extraction, implemented `trackPaletteProvider` as FutureProvider for non-blocking background color extraction, extended `ActiveTrackEntry` with `albumArtUrl` field, wired palette watch into `CinematicPlayerScreen` to dynamically color gradient overlay and play button via `AnimatedContainer` (500ms gradient transition, 300ms button transition), fallback gracefully to OSTrack Gold while extracting colors, added comprehensive `PALETTE_GENERATOR_INTEGRATION.md` guide with performance profile, animation timings, and Phase 2+ enhancement roadmap.
- 2026-04-10 — Integrated mascot backend API path: added a Supabase-backed `MascotRepository` with local preference fallback for offline resilience, wired Riverpod `mascotCatalogViewProvider` and `MascotController` for fetch/equip/purchase flows, and connected `MascotCabinetScreen` to backend-driven ownership/equipped state with in-screen purchase action that syncs `user_mascots` and `users.equipped_mascot_id` when available.
- 2026-04-10 — Added reusable mascot-anchored avatar UI: created `UserAvatarWithMascot` widget using a `Stack` to overlay the equipped pixel mascot on the user avatar, sourced equipped mascot state from Riverpod (`mascotCatalogViewProvider`), and integrated it into the Home header with notification action plus tap-to-Profile-tab behavior for quick access to cabinet/profile.
- 2026-04-10 — Replaced plain user avatars across app surfaces with `UserAvatarWithMascot`: updated Home feed identity chips and onboarding suggestion rows to use the reusable mascot-anchored component, removing legacy initials-only avatar rendering so mascot identity is consistently visible in key social entry points.
- 2026-04-10 — Fixed Android build compatibility with AGP 8.11 for Isar plugins: added root Gradle Kotlin DSL safeguards to inject fallback namespaces for Android library subprojects and auto-patch `isar_flutter_libs` manifest package attribute during configuration, then validated with `flutter build apk --debug` producing a successful `app-debug.apk` output.
- 2026-04-10 — Fixed top-screen red line artifact in app shell: updated `OfflineBoundary` hide/show animation from fixed negative positioning to `AnimatedSlide` + `IgnorePointer`, ensuring the offline banner is fully off-canvas when hidden across devices with varying status-bar insets.
- 2026-04-10 — Finalized core interaction glue for beta UX loops: added a dedicated cabinet `Store` button with slide-up mascot purchase modal (simulated pricing and instant ownership state update via `mascotControllerProvider`) and implemented Scene Timeline contribution bottom sheet in `MediaSourcePage` with fidelity mode selection (Description-only vs. Timestamp+Description), required description input, optional timestamp capture, and immediate in-memory timeline insertion with submission feedback.
- 2026-04-10 — Implemented premium Fuzzy-Logic Scene Tag modal: added reusable `SceneTagSubmissionModal` widget with fidelity tiers (`Memory`, `Chapter`, `Exact`), adaptive context field (episode/level vs. timestamp), keyboard-aware editorial sheet styling, and submission feedback; wired trigger action into `CinematicPlayerScreen` scene-tag section via `showModalBottomSheet` for immediate in-player contributions.
- 2026-04-10 — Applied editorial polish pass across Home Feed and mini-player: upgraded feed cards with deeper vertical gradients and tighter modern radius, tuned content hierarchy with stronger track-title emphasis and lower-opacity secondary metadata text, added subtle mascot color bloom in `UserAvatarWithMascot`, converted mini-player to frosted-glass blur treatment with dynamic palette-driven glowing progress strip, and tightened home stagger reveal timing to 50ms for magazine-style list entry.
- 2026-04-10 — Fixed shelves repository diagnostics: removed redundant null-assert operators in Supabase shelf sync methods after explicit null guards, restoring clean analyzer status for `shelf_repository.dart` and reducing risk of nullable-flow regressions in shelf fetch paths.
- 2026-04-10 — Applied strict content-constraint cleanup for information density: enforced one-line ellipsis treatment across key list/text surfaces (search results, trending, hidden gems, tracklist rows), consolidated secondary metadata presentation with concise dot-separated lines, added `ClipRect`-bounded mini-player blur composition to improve visual layering, and moved active-track identity text onto artwork with a bottom gradient overlay for premium hero-card readability.
- 2026-04-10 — Applied cinematic shell pass for stronger glassmorphism: moved blur treatment to the entire bottom shell (`NavigationBar` + mini-player) with translucent backdrop bleed-through, and converted Home feed activity blocks into compact one-line editorial rows with strict ellipsis and dot-separated metadata.
- 2026-04-10 — Hardening Sprint A completed for core reliability: converted `isarProvider` to non-null contract with explicit startup override requirement, moved Isar boot failure handling into `main()` with a graceful local error screen fallback, upgraded connectivity stack for plugin compatibility, replaced radio-only offline detection with interface + live internet reachability checks in `OfflineBoundary`, and removed duplicate local `PendingMutation` Isar model so `IsarPendingMutation` remains the single queue source of truth.
- 2026-04-10 — Hardening Sprint B completed for shell decomposition: extracted routing/tab orchestration into dedicated `OstrackNavigationShell`, extracted mini-player playback UI into reusable `FrostedMiniPlayer`, and replaced duplicated feed row logic with reusable `SharedEditorialListItem`; rewired home route to use the new navigation shell and removed legacy in-file `_MiniPlayerBar`/`_FeedCard` implementations from `ostrack_shell.dart`.
- 2026-04-10 — Hardening Sprint C completed for moderation/compliance baseline: added report actions to scene-tag UI surfaces (Media Source timeline cards + cinematic player tag panel), introduced `SceneTagModerationRepository` with Supabase-backed report persistence plus fallback counts and auto-hide threshold logic (>2 reports), hid reported timeline entries pending review, and documented an explicit metadata boundary that restricts stored catalog metadata to open/community sources while keeping Spotify/Apple integrations runtime-only.
- 2026-04-10 — Hardening Sprint D completed for moderation operations: expanded scene-tag moderation repository with queue fetch + hide/restore APIs, added dedicated `SceneTagModerationScreen` for trusted-review workflows, exposed moderation routing (`/moderation`) and in-profile access entrypoint, and enabled refreshable review of flagged tags with explicit visibility controls.
- 2026-04-10 — Hardening Sprint E completed for moderation governance: added persisted moderation-access permission in `AppPreferences`, enforced route-level access gating for `/moderation`, added settings toggle to explicitly grant moderation tools access, restricted profile moderation entrypoint to authorized users, and added moderation action audit logging with recent action history surfaced in the moderation queue screen.
- 2026-04-12 — Mascot animation compliance/performance fix: made `MascotSpriteView` animation opt-in via new `animated` flag (default false), stopped unconditional ticker repeat for non-animated sprites, and enabled idle animation only for equipped mascots in cabinet surfaces (`mascot_cabinet_screen.dart` grid + `mascot_monetization.dart` cabinet case tiles) to align with spec and reduce concurrent animation controllers.
- 2026-04-12 — Cabinet locked-slot interaction fix: added `onOpenStoreForMascot(String mascotId)` callback plumbing through `MascotCabinetSection` and `_MascotCaseTile`, made locked tiles tap-to-store instead of inert, and added focused store entry (`initialHighlightedMascotId`) so opening from a locked tile pre-filters to that mascot tier and highlights the targeted mascot card.
- 2026-04-12 — Removed dead emoji fallback helper `mascotGlyphForId` from `mascot_monetization.dart`; mascot rendering now relies solely on `MascotSpriteView` to keep the codepath aligned with the current sprite-based cabinet/store UI.
- 2026-04-12 — Featured mascot drop visual upgrade: added a large animated `MascotSpriteView` hero (inside `_FeaturedMascotDrop`) so the store drop card now showcases the mascot at full-size motion instead of text-only metadata.
- 2026-04-12 — Store purchase action cleanup: disabled `_MascotStoreCard` purchase button when a mascot is already owned so the Owned state is non-actionable and no longer emits a redundant purchase snackbar.
- 2026-04-12 — Store state synchronization fix: added `didUpdateWidget` to `MascotStorePage` so local preferences refresh when the parent rebuilds, keeping owned/equipped badges in sync while the store route remains open.
- 2026-04-12 — Mascot painter transform hardening: wrapped the `_drawSkeleton` bounce offset in `canvas.save()` / `canvas.restore()` so the translation stays scoped to the skeleton draw and cannot leak to future paint additions.
- 2026-04-12 — Store CTA simplification: collapsed `_MascotStoreCard` into a single contextual action button that transitions from `Add to Collection` to `Equip` and then `Equipped ✓`, removed the redundant separate equip action, and added a TODO to source limited-edition numbers from the eventual purchase record instead of the current seeded placeholder value.
